  ...
  <html>
    <head>
      <title>
        ...
        <xsl:text>: </xsl:text>
        <xsl:value-of select="upper-case(substring(string(./tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title[1]),1,1))"/>
    <xsl:value-of
        select="substring(string(./tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title[1]),2)"/>
      </title>
      <link type="text/css" rel="stylesheet"
            href="view_transcript.css"/> 
    ...
    <body>
      <div xmlns="http://www.w3.org/1999/xhtml" id="style_switches" class="switches">
	  <span class="switch sw_line_no" onclick="toggleClass('line_no');" title="utterance identifier">1</span>
	  <span class="switch sw_sid" onclick="toggleClass('sid');" title="Speaker id">S1:</span> <span class="switch   sw_overlap_tag" onclick="toggleClass('overlap_tag');" title="overlap tags">&lt;1&gt;</span> <span class="switch   sw_pause" onclick="toggleClass('pause');" title="pauses">(.)</span> <span class="switch   sw_contextual_event" onclick="toggleClass('contextual_event');" title="contextual events">{...}</span> 
	  <span class="switch sw_smode" onclick="toggleClass('smode');" title="speaking modes">&lt;fast&gt;</span>
	  <span class="switch sw_vocal" onclick="toggleClass('vocal');" title="vocal/speaker noise">&lt;coughs&gt;</span>
	  <span class="switch sw_spel" onclick="toggleClass('spel_tag');" title="spelled">&lt;spel&gt;</span>
	  <span class="switch sw_foreign_tag" onclick="toggleClass('foreign_tag');" title="foreign language tags">&lt;L1fr&gt;</span> <span class="switch        sw_other_continuation" onclick="toggleClass('other_continuation');" title="other continuations">=</span> <span class="switch sw_un_tag" onclick="toggleClass('un_tag');" title="unintelligible tags">&lt;un&gt;</span> <span class="switch sw_ono_tag" onclick="toggleClass('ono_tag');" title="onomatopoeia">&lt;ono&gt;</span> <span class="switch sw_pvc_tag" onclick="toggleClass('pvc_tag');" title="pvc tags">&lt;pvc&gt;</span>
	  <span class="switch sw_gap" onclick="toggleClass('gap');" title="gap">(gap/(nrec</span>
	</div>
...
<xsl:template match="tei:textClass">
  ...
  <table class="textClass header">
    <xsl:for-each select="tokenize(normalize-space(tei:catRef[1]/@target),' ')">
      <tr>
	<th><xsl:value-of select="id(substring-after(., '#'),$tc)/../tei:catDesc"/>: </th>
	<td><xsl:value-of select="id(substring-after(.,'#'),$tc)"/></td>
      </tr>
    </xsl:for-each>
  </table>
  </div>
...
<xsl:template match="tei:setting">
  <table class="setting header">
    <xsl:if test="tei:name[@type='country']">
      <tr>
	<th>Country-Code: </th>
	<td><xsl:value-of select="tei:name[@type='country']"/></td>
      </tr>
    </xsl:if>
    <xsl:if test="tei:name[@type='city']">
      <tr>
	<th>City: </th>
	<td><xsl:value-of select="tei:name[@type='city']"/></td>
      </tr>
    </xsl:if>
    <xsl:if test="tei:locale">
      <tr>
	<th>Locale: </th>
	<td><xsl:apply-templates select="tei:locale"/></td>
      </tr>
    </xsl:if>
    <xsl:if test="tei:activity">
      <tr>
	<th>Activity: </th>
	<td><xsl:apply-templates select="tei:activity"/></td>
      </tr>
    </xsl:if>
  </table>
</xsl:template>
...
<xsl:template match="tei:listPerson[child::*]">
  <xsl:variable name="tw" as="xs:integer">
	...
	<xsl:choose>
	  ...
	  <xsl:when test="@type='identified'"><xsl:text>Identified</xsl:text></xsl:when>
	  <xsl:when test="@type='not_identified'"><xsl:text>Speakers Not Identified</xsl:text></xsl:when>
...
<xsl:template match="tei:bibl/tei:title[@level='m']" priority="1">
  <span class="ref in"><xsl:apply-templates/></span>
</xsl:template>

<xsl:template match="tei:ref">
  <a>
	<xsl:attribute name="target">_blank</xsl:attribute>
    <xsl:if test="@target"><xsl:attribute name="href"><xsl:value-of select="@target"/></xsl:attribute></xsl:if>
    <xsl:choose>
      <xsl:when test="*|text()">
	<xsl:apply-templates/>
      </xsl:when>
...
<xsl:template match="/cq:corpusHeader/tei:teiHeader" name="corpusHeader">
  ...
  <div class="title">
    ...
    <xsl:if test=".//tei:principal">
      ...
      <ul>
      <li><xsl:value-of select="xs:string(.//tei:principal)"/></li>
      </ul>
    </xsl:if>
    <xsl:if test=".//tei:titleStmt/tei:respStmt">
      <xsl:for-each select=".//tei:titleStmt/tei:respStmt/tei:resp">
		...
		<h3><xsl:value-of select="xs:string($resp)"/></h3>
		<ul>
		  <xsl:for-each select="./following-sibling::tei:name">
			<li><xsl:value-of select="xs:string(.)"/></li>
		  </xsl:for-each>
		</ul>
      </xsl:for-each>
    </xsl:if>
    <xsl:if test=".//tei:funder">
      ...
      <ul>
	<li><xsl:value-of select="xs:string(.//tei:funder)"/></li>
...
<!--	    </ul> -->
  ...
  <xsl:if test=".//tei:classDecl//tei:category[@xml:id = 'domain']">
	...
	<dl>
	  <xsl:apply-templates mode="corpusHeader"
						 select=".//tei:classDecl//tei:category[@xml:id
								 = 'domain']"/>
	</dl>
  </xsl:if>
  ...
  <xsl:if test="not(.//tei:classDecl//tei:category[@xml:id = 'domain' or @xml:id = 'spet'])">
	...
	<dl>
    <xsl:apply-templates mode="corpusHeader"
						 select=".//tei:classDecl/tei:taxonomy/tei:category"/>
	</dl>
  </xsl:if>
  ...
  <xsl:if test=".//tei:classDecl//tei:category[@xml:id = 'first_languages' or @xml:id = 'person_roles' or @xml:id = 'person_groups' or @xml:id='power_relations' or @xml:id='acquaintedness']">
	...
	<xsl:if test=".//tei:particDesc/tei:listPerson[@type='unidentified']">
	  ...
	  <dl class="speakers">
	  <xsl:apply-templates
		  select=".//tei:particDesc/tei:listPerson[@type='unidentified']"
		  mode="corpusHeader"/>
	  </dl>
	</xsl:if>
...
<xsl:template match="tei:p" mode="corpusHeader">
  <p><xsl:apply-templates mode="corpusHeader"/></p>
</xsl:template>

<xsl:template match="tei:p" mode="corpusHeaderPersonDesc">
  <dd><xsl:apply-templates mode="corpusHeaderPersonDesc"/></dd>
</xsl:template>
  ...
  <xsl:choose>
	...
	<xsl:otherwise>
	  <xsl:element name="{$type}">
		<xsl:for-each select="tei:item">
		  <li><xsl:apply-templates mode="corpusHeader"/></li>
		</xsl:for-each>
	  </xsl:element>
	</xsl:otherwise>
  </xsl:choose>
...
<!--  <xsl:message>TEI:U</xsl:message> -->
  ...
  <xsl:element name="{$uelem}">
	...
	select="$u_classes"/></xsl:attribute>
	<xsl:attribute name="id"><xsl:value-of select="$uid"/>
	</xsl:attribute>
...
<!--	  <xsl:value-of select="$uid_sel"/> -->
	...
	<xsl:element name="{$upartelem}">
	  <xsl:attribute name="class"><xsl:value-of
	  select="$sid_classes"/></xsl:attribute>
	  <xsl:attribute name="id">
		<xsl:value-of
			select="concat('src_', $uid, '_', substring-after(@who,'#'))"/>
	  </xsl:attribute>
...
<xsl:template match="tei:supplied[@reason='unintelligible']">
  ...
  <span class="un_tag"><xsl:text>&lt;un&gt;</xsl:text></span>
  <xsl:apply-templates/>
...
<xsl:template match="tei:incident">
  <span class="contextual_event">
    ...
    <xsl:if test="./@dur">
      <xsl:text> (</xsl:text>
      <xsl:value-of
          select="substring-before(substring-after(string(./@dur),'PT'),'S')"/>
      <xsl:text>)</xsl:text>
    </xsl:if>
...
<xsl:template match="tei:pause">
  <span class="pause"><xsl:text> (</xsl:text>
  <xsl:choose>
    <xsl:when test="./@dur">
...
<xsl:template name="gapdesc">
  <xsl:if test="@voice:desc and @voice:desc ne ''">
    <span>
      <xsl:text>{</xsl:text>
      <xsl:value-of select="@voice:desc"/>
      <xsl:text>}</xsl:text>
  ...
  <xsl:template name="extract_time">
    ...
    <xsl:value-of select="$h"/><xsl:text>:</xsl:text>
